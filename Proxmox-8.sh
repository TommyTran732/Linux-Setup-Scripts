...
curl https://raw.githubusercontent.com/GrapheneOS/infrastructure/main/systemd/system/sshd.service.d/local.conf | sudo tee /etc/systemd/system/ssh.service.d/override.conf
...
sudo mkdir -p /etc/systemd/system/pve-daily-update.service.d
echo '[Service]
ExecStart=/usr/bin/pveupgrade' | sudo tee /etc/systemd/system/pve-daily-update.service.d/override.conf
sudo systemctl daemon-reload
sudo systemctl enable --now pve-daily-update.timer
